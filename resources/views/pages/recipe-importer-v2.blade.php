...
use App\Services\RmsV2\RmsValidationResult;
...
new class extends Component
{
    ...
    public function preview()
    {
        ...
        if (!$result->valid) {
            ...
            // Build summary of valid sections found
            $this->validSections = [
        ...
        <div>
            <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                ...
                <textarea
                    wire:model="markdown"
                    rows="25"
                    placeholder="Pega aquí tu receta en formato RMS v2.0...
                ...
                <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden sticky top-20">
                    ...
                    <div class="px-5 py-4 space-y-3 max-h-[600px] overflow-y-auto text-sm">
                        ...
                        @if(!empty($parsedData['ingredients']))
                            <div class="pt-2">
                                <span class="text-xs text-gray-400 uppercase tracking-wide">Ingredientes</span>
                                <ul class="mt-1 space-y-1">
                                    @foreach(array_slice($parsedData['ingredients'], 0, 8) as $ing)
                                        <li class="text-gray-600 flex items-center gap-1">
                                            <span class="text-gray-300">•</span>
                                            @if($ing['quantity']) <strong>{{ $ing['quantity'] }}</strong> {{ $ing['unit'] }} @endif
                                            {{ $ing['name'] }}
                                            <span class="text-xs text-gray-400">[{{ $ing['category'] }}]</span>
                                        </li>
                                    @endforeach
                                    @if(count($parsedData['ingredients']) > 8)
                                        <li class="text-xs text-gray-400">... y {{ count($parsedData['ingredients']) - 8 }} más</li>
                                    @endif
                                </ul>
                            </div>
                        @endif
                        ...
                        @if(!empty($parsedData['steps']))
                            <div class="pt-2">
                                <span class="text-xs text-gray-400 uppercase tracking-wide">Pasos</span>
                                @foreach(array_slice($parsedData['steps'], 0, 3) as $i => $step)
                                    <div class="mt-1 flex gap-2">
                                        <span class="text-purple-600 font-bold text-xs flex-shrink-0">{{ $i + 1 }}.</span>
                                        <span class="text-gray-600">{{ Str::limit($step['action'], 100) }}</span>
                                    </div>
                                @endforeach
                                @if(count($parsedData['steps']) > 3)
                                    <p class="text-xs text-gray-400 mt-1">... y {{ count($parsedData['steps']) - 3 }} más</p>
                                @endif
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="px-5 py-4 border-t border-gray-200 bg-gray-50 rounded-b-2xl">
                        ...
                        <p class="text-xs text-gray-400 text-center mt-2">Validación aprobada. Se creará la receta con todos sus datos.</p>
                    </div>
                </div>
            </div>
        @endif
